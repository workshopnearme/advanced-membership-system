...
jQuery(document).ready(function($) {
    ...
    $('#ams-fields-list').sortable({
        ...
        update: function() {
            $('#ams-save-status').html('<span style="color: orange;">⚠ <?php _e('Unsaved changes', 'ams'); ?></span>');
        }
    });
    ...
    $('#ams-add-field-btn').on('click', function() {
        ...
        if (!fieldId || !fieldLabel) {
            alert('<?php _e('Please fill in Field ID and Label', 'ams'); ?>');
            return;
        }
    ...
    $(document).on('click', '.delete-field', function() {
        if (!confirm('<?php _e('Are you sure you want to delete this field?', 'ams'); ?>')) {
            return;
        }
    ...
    $('#ams-save-fields-btn').on('click', function() {
        ...
        $.post(ajaxurl, {
            ...
            if (response.success) {
                $('#ams-save-status').html('<span style="color: green;">✓ <?php _e('Saved successfully!', 'ams'); ?></span>');
                setTimeout(function() {
                    $('#ams-save-status').fadeOut();
                }, 3000);
            } else {
                $('#ams-save-status').html('<span style="color: red;">✗ <?php _e('Error saving', 'ams'); ?></span>');
            }
        });
    });
});
